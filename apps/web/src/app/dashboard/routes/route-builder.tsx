...
export function RouteBuilder({
  ...
  useEffect(() => {
    setPage(1);
  }, [search, driverId, warehouseId, status, routeDate]);
  ...
  const hasFilters = Boolean(
    search || driverId || warehouseId || status || routeDate !== today,
  );

  const todayRoutes = routes.filter((route) => route.route_date === today);

  const scheduledToday = todayRoutes.filter(
    (route) => route.status === "scheduled",
  ).length;

  const completedToday = todayRoutes.filter(
    (route) => route.status === "completed",
  ).length;

  const stopsToday = todayRoutes.reduce(
    ...
    <>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm leading-6 text-steam-400">
              Review generated and manually created routes, manage stops, and
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full bg-espresso-950 px-5 py-2.5 text-sm font-medium text-crema-50 transition-colors hover:bg-copper-600"
          >
            {showCreateForm ? (
            ...
            {showCreateForm ? "Close" : "Create one-off route"}
          </button>
        </div>

        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Routes today" value={todayRoutes.length} />

          <MetricCard label="Scheduled" value={scheduledToday} />

          <MetricCard label="Completed" value={completedToday} />

          <MetricCard label="Stops today" value={stopsToday} />
        </div>
      </section>
        ...
        <section className="mb-8 rounded-2xl border border-latte-200 bg-crema-0 p-6 shadow-sm">
          <div className="mb-5">
            <h3 className="text-lg font-semibold text-espresso-950">
              Create one-off route
            </h3>

            <p className="mt-1 text-sm text-steam-400">
              Use this for an operational route that should not change a
      ...
      <section>
        ...
        <div className="mb-5 grid gap-3 rounded-2xl border border-latte-200 bg-crema-0 p-4 shadow-sm md:grid-cols-2 xl:grid-cols-5">
          ...
          <input
            aria-label="Filter by route date"
            type="date"
            value={routeDate}
        ...
        {visibleRoutes.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-latte-200 bg-crema-0 p-10 text-center">
            <p className="font-medium text-espresso-950">No routes found</p>

            <p className="mt-1 text-sm text-steam-400">
              No operational routes match the selected filters.
            </p>
          </div>
        ) : (
