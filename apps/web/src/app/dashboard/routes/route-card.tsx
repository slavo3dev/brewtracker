...
const orderInputClass =
  "w-20 rounded-xl border border-latte-200 bg-crema-0 px-3 py-2 text-sm text-espresso-950 outline-none focus:border-copper-500 focus:ring-2 focus:ring-copper-500/20";
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-latte-200 bg-crema-0 shadow-sm">
      <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        >
          <span className="mt-0.5 grid size-10 shrink-0 place-items-center rounded-full bg-latte-100 text-espresso-800">
            <UsersIcon className="size-5" />
          </span>
          <span className="min-w-0">
            <span className="block truncate text-lg font-semibold text-espresso-950">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-steam-400">
              ...
              <span>
                {sortedStops.length}{" "}
                {sortedStops.length === 1 ? "stop" : "stops"}
              </span>
              <span
                className={[
              ...
              >
                {route.source_route_template_id ? "Recurring" : "One-off"}
              </span>
            </span>
            {route.notes && (
              <span className="mt-1 block truncate text-sm text-steam-400">
                {route.notes}
              </span>
            )}
          </span>
        ...
        <div className="flex items-center justify-between gap-3 lg:justify-end">
          ...
          <button
            type="button"
            onClick={() => setIsExpanded((value) => !value)}
            className="inline-flex items-center gap-2 rounded-full border border-latte-200 px-4 py-2 text-sm font-medium text-espresso-800 transition-colors hover:bg-latte-100"
          >
        ...
        <div className="border-t border-latte-200 bg-latte-100/40 p-5">
          ...
          <div className="mb-3 mt-5 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-steam-400">
                Manage route order and scheduled visits.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 grid gap-3">
            {sortedStops.length === 0 ? (
              <div className="rounded-xl border border-dashed border-latte-200 bg-crema-0 p-5 text-sm text-steam-400">
    ...
    <div className="rounded-xl border border-latte-200 bg-crema-0 p-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="grid size-7 place-items-center rounded-full bg-espresso-950 text-xs font-semibold text-crema-50">
              {stop.sequence_number}
            </span>
            <p className="font-medium text-espresso-950">
              {stop.client?.name ?? "Unknown client"}
            </p>
            <span className="rounded-full bg-latte-100 px-2.5 py-1 text-xs font-medium capitalize text-espresso-800">
              {stop.status}
            </span>
          </div>
          ...
          <p className="mt-1 text-sm text-steam-400">
            Scheduled: {formatDate(stop.scheduled_start_at)} →{" "}
            {formatDate(stop.scheduled_end_at)}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 lg:justify-end">
          <form action={sequenceAction} className="flex items-center gap-2">
            ...
            <input
              aria-label={`Order for ${stop.client?.name ?? "stop"}`}
              name="sequenceNumber"
              type="number"
              min="1"
          ...
          <form
            ...
            onSubmit={(event) => {
              if (
                !window.confirm(
                  `Remove ${stop.client?.name ?? "this stop"} from the route?`,
                )
              )
      ...
      {feedback && (
        <p
          ...
          className={`mt-3 rounded-lg px-3 py-2 text-sm ${hasError ? "bg-copper-100 text-copper-600" : "bg-latte-100 text-espresso-800"}`}
        >
          {feedback}
        </p>
      )}
